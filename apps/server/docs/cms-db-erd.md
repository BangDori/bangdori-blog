...
이번 PR에서 실제 생성하는 테이블은 `posts` 하나다. `external_posts`, `site_daily_stats`는 후속 PR에서 별도로 구현한다.
...
`view_count`는 글 상세/목록에서 함께 조회할 글별 누적 조회수다. 조회수 증가 시 글 수정 시각인 `updated_at`이 바뀌지 않도록 별도 update 쿼리로 다룬다.
