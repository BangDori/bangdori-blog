...
export function publishExternalPost(id: string): Promise<ExternalPost> {
  return api.post<ExternalPost>(`/admin/external-posts/${id}/publish`);
}

export function archiveExternalPost(id: string): Promise<ExternalPost> {
  return api.post<ExternalPost>(`/admin/external-posts/${id}/archive`);
}
